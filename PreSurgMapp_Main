function [Error]=TumorProcessing_main(AutoDataProcessParameter)
% FORMAT [Error]=TumorProcessing_main(AutoDataProcessParameter)
% Input:
...
% Last Modified by Huiyuan Huang on 25-Jan-2016
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Reorient Interactively After Coregistration for better orientation in Segmentation      Added by HHY 29150603
%Do not need parfor
...
if (AutoDataProcessParameter.IsCovremove==1)
    ...
    for i=1:AutoDataProcessParameter.SubjectNum
        ...
        global TumorProcessing_spm_image_Parameters
        TumorProcessing_spm_image_Parameters.tag=1;
        TumorProcessing_spm_image_Parameters.ReorientFileList=FileList;
        
        for j=1:20  % support for more covariate selecting,added by Huiyuan Huang 20140815
            
            
            uiwait(TumorProcessing_spm_image('init',[AutoDataProcessParameter.DataProcessDir,filesep,'T1ImgCoreg',filesep,AutoDataProcessParameter.SubjectID{i},filesep,DirT1ImgCoreg(1).name]));
            if TumorProcessing_spm_image_Parameters.tag==0
                break
                
            end
            mat=TumorProcessing_spm_image_Parameters.ReorientMat;
            ...
            %  clear global TumorProcessing_spm_image_Parameters
...
if (AutoDataProcessParameter.IsCalFC==1)
    ...
    for i=1:AutoDataProcessParameter.SubjectNum
        ...
        global TumorProcessing_spm_image_ROI_Parameters
        TumorProcessing_spm_image_ROI_Parameters.tag=1;
        TumorProcessing_spm_image_ROI_Parameters.T1FileList=FileList;
        
        for jj=1:20  % support for more ROI selecting,added by Huiyuan Huang 20140815
            
            uiwait(TumorProcessing_spm_image_ROI('init',[AutoDataProcessParameter.DataProcessDir,filesep,'T1ImgCoreg',filesep,AutoDataProcessParameter.SubjectID{i},filesep,DirT1ImgCoreg(1).name]));
            if  TumorProcessing_spm_image_ROI_Parameters.tag==0
                break
            end
            mat= TumorProcessing_spm_image_ROI_Parameters.ROIMat;
...
%% Added by HHY 20130604
%%%%%%% Automatic indentify task-related independent component %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if (AutoDataProcessParameter.IsCaltaskactivation==1)&&(AutoDataProcessParameter.IsCaltaskICA==1)
    Impulsefunction=zeros(1,AutoDataProcessParameter.TimePoints);
    if AutoDataProcessParameter.taskactivation.Duration==0
        onset=AutoDataProcessParameter.taskactivation.Onset';
        interval=0;
    else
        onset=AutoDataProcessParameter.taskactivation.Onset';
        interval=AutoDataProcessParameter.taskactivation.Duration-1;
    end
    ...
    if ~isempty(strfind(AutoDataProcessParameter.StartingDirName,'NiftiPairs'))
        ...
        % Extract time course
        for ss=3:length(sublist)
            % TumorTCdir=dir([ICADir,filesep,sublist(ss).name,filesep,'output',filesep,'gc_sub001_timecourses_ica_s1_.hdr']);
...
if (AutoDataProcessParameter.IsCalICA==1)&&(AutoDataProcessParameter.IsCalDICIICA==1)
    ...
    for i =1:AutoDataProcessParameter.SubjectNum
        ...
        a = zeros(64,64,AutoDataProcessParameter.SliceTiming.SliceNumber); %改
        a(mask_ind)=1;
        % [data head]= rest_ReadNiftiImage('D:\dataProcessing\ZhenZhouUnivNo1Hosp\analysis\intra-ICA-80\output\gc_sub001_component_ica_s1_002.hdr');
        ...
        % rest_WriteNiftiImage(a,head,'D:\dataProcessing\ZhenZhouUnivNo1Hosp\analysis\intra-ICA-80\output\mask.img');
    ...
    for i =1:AutoDataProcessParameter.SubjectNum
        ...
        % read binary task activation gen by rest (p<1e-10, FWE, single tailed, t > 10.75; extent: clust size > 20)
        % note: this binary mask is already intersected with MICA_brain_mask
        %[dataTumor headTumor] = rest_ReadNiftiImage([homeDir,'/wm_task_activation/','binary_in_mask.hdr']);
        [dataSMN headSMN] = rest_ReadNiftiImage(SMNDir); %  individual Tumor directory
        
        % read brain mask file in MICA (MICA_brain_mask)
        %[dataMask headMask] = rest_ReadNiftiImage([homeDir,'/wm_task_activation/','brainmask_n.hdr']);
        ...
        for tt =1:length(tcn)
            ...
            cd([homeDir,'\','output']);
            mkdir('temp');
            % unzip('gc_tmap__component_ICA_.zip','temp/');
            ...
            for aa = 1:length(a)
                ...
                smallClusterID = find(clusterSizeAll<20); % cluster threshold set to be 20 要不要改？
                ...
                % 2. faulse alarm rate
                temp = zeros(64,64,AutoDataProcessParameter.SliceTiming.SliceNumber); % 改
    ...
    %     cd([AutoDataProcessParameter.DataProcessDir,filesep,'DICIresult']);
    %     save  DICI_allsubject_SMN  DICI_allsubject_SMN
    ...
    for i =1:AutoDataProcessParameter.SubjectNum
        ...
        ICADir=([ICADir,'\',AutoDataProcessParameter.SubjectID{i}]);
        maskDir =([maskDir ,'\',AutoDataProcessParameter.SubjectID{i},'_mask.hdr']);
        LNDir=([LNDir ,'\',AutoDataProcessParameter.SubjectID{i},'\','wPositiveBinaryMask_template_languageFDR001.hdr']); % 改
        DICIresultDir=([DICIresultDir ,'\',AutoDataProcessParameter.SubjectID{i},'_DICI_Language']);
        
        % read binary task activation gen by rest (p<1e-10, FWE, single tailed, t > 10.75; extent: clust size > 20)
        % note: this binary mask is already intersected with MICA_brain_mask
        %[dataTumor headTumor] = rest_ReadNiftiImage([homeDir,'/wm_task_activation/','binary_in_mask.hdr']);
        [dataLN headLN] = rest_ReadNiftiImage(LNDir); %  individual Tumor directory
        ...
        %         DICI_allsubject_Language{i,1}=finalResult_LN; %改
    ...
    %     cd([AutoDataProcessParameter.DataProcessDir,filesep,'DICIresult']);
    %     save  DICI_allsubject_Language  DICI_allsubject_Language
    
end
%%%%%%%%%%%%%%%% Task activation %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
clear matlabbatch jobfile jobs
if (AutoDataProcessParameter.IsCaltaskactivation==1)
    ...
    homeStat =([AutoDataProcessParameter.DataProcessDir,filesep,AutoDataProcessParameter.taskname,'_activation']); %改：输出路径
    homeHM = ([AutoDataProcessParameter.DataProcessDir,filesep,'RealignParameter']); %改：头动参数所在文件夹
    
    %     if AutoDataProcessParameter.FunctionalSessionNumber==1
    %         homeDat =([AutoDataProcessParameter.DataProcessDir,filesep,'NiftiPairs']);
    %     else
    if ~isempty(strfind(AutoDataProcessParameter.StartingDirName,'NiftiPairs'))                    % revised by HHY 20130603
        homeDat =([AutoDataProcessParameter.DataProcessDir,filesep,AutoDataProcessParameter.StartingDirName]); %改：平滑后数据所在文件夹
    elseif AutoDataProcessParameter.FunctionalSessionNumber==1
        homeDat=([AutoDataProcessParameter.DataProcessDir,filesep,'NiftiPairs']);
    elseif AutoDataProcessParameter.FunctionalSessionNumber>1
    ...
    for subID =1:length(subName)
        ...
        matlabbatch{1}.spm.stats.fmri_spec.sess.cond(1).name = AutoDataProcessParameter.taskname; %改： 结果名字
        matlabbatch{1}.spm.stats.fmri_spec.sess.cond(1).onset = AutoDataProcessParameter.taskactivation.Onset; %% load onset
        matlabbatch{1}.spm.stats.fmri_spec.sess.cond(1).duration = AutoDataProcessParameter.taskactivation.Duration;        %改： duration
        ...
        temp = dir([homeHM,'/',HMsubName(subID,1).name,'/',FunSessionPrefixSet{iFunSession},'rp_*.txt']);  %改： 头动参数
