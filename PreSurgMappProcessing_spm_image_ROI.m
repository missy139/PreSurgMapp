function fg=PreSurgMappProcessing_spm_image_ROI(op,varargin)
% image and header display
% FORMAT PreSurgMappProcessing_spm_image_ROI
%_______________________________________________________________________
%
% PreSurgMappProcessing_spm_image is an interactive facility that allows orthogonal sections
...
global st
global PreSurgMappProcessing_spm_image_ROI_Parameters %Huiyuan Huang 2014.0815
global fg  
...
if nargin == 0,
    ...
    PreSurgMappProcessing_spm_image_ROI('init',P);
    return;
end;

if isstruct(op)
    % job data structure
    PreSurgMappProcessing_spm_image_ROI('init', op.data{1});
    return;
end;
...
if strcmp(op,'repos'),
    ...
    % spm_orthviews('MaxBB');
    PreSurgMappProcessing_spm_image_ROI('zoom_in');
    PreSurgMappProcessing_spm_image_ROI('update_info');
...
if strcmp(op,'addblobs'),
    ...
    for i=1:nblobs,
        ...
        set(st.blobber,'String','Remove Blobs','Callback','PreSurgMappProcessing_spm_image_ROI(''rmblobs'');');
    end;
...
if strcmp(op,'rmblobs'),
    ...
    set(st.blobber,'String','Add Blobs','Callback','PreSurgMappProcessing_spm_image_ROI(''addblobs'');');
...
if strcmp(op,'defineROI'),
    ...
    PreSurgMappProcessing_spm_image_ROI_Parameters.ROIMat = mat;
     mat=eye(4);
     
%     if det(mat)<=0
%         spm('alert!','This will flip the images',mfilename,0,1);
%     end;
    P = PreSurgMappProcessing_spm_image_ROI_Parameters.T1FileList;
...
%         DPARSFA_spm_image('init',st.vols{1}.fname);
%     end;
    close(fg);
...
if strcmp(op,'Done'),
    ...
    %----------------------------------------------------------------------
    close(fg);
    PreSurgMappProcessing_spm_image_ROI_Parameters.tag=0;
    return;
end;
...
if strcmp(op,'resetorient'),
    ...
    tmp = spm_get_space([st.vols{1}.fname ',' num2str(st.vols{1}.n)]);
    if sum((tmp(:)-st.vols{1}.mat(:)).^2) > 1e-8,
       PreSurgMappProcessing_spm_image_ROI('init',st.vols{1}.fname);
    end;
    return;
end;
...
if isempty(fg), error('Can''t create graphics window'); 
end
spm_figure('Clear','Graphics');

P = varargin{1};
%  If no images specified, then reorient itself.
if ~isfield(PreSurgMappProcessing_spm_image_ROI_Parameters,'T1FileList')
   PreSurgMappProcessing_spm_image_ROI_Parameters.T1FileList={P};
end
...
spm_orthviews('MaxBB');
st.callback = 'PreSurgMappProcessing_spm_image_ROI(''shopos'');';
...
%-----------------------------------------------------------------------
uicontrol(fg,'Style','Frame','Position',[60 25 200 325].*WS,'DeleteFcn','PreSurgMappProcessing_spm_image_ROI(''reset'');');

uicontrol(fg,'Style','Pushbutton','String','Define ROI','Callback','PreSurgMappProcessing_spm_image_ROI(''defineROI'');',...
         'Position',[70 35 100 020].*WS,'ToolTipString','define ROI directly from the anatomy image');
% uicontrol(fg,'Style','Pushbutton','String','Reorient images','Callback','DPARSFA_spm_image(''reorient'')',...
%          'Position',[70 35 125 020].*WS,'ToolTipString','modify position information of selected images');
     
uicontrol(fg,'Style','Pushbutton','String','Done','Callback','PreSurgMappProcessing_spm_image_ROI(''Done'');',...
         'Position',[175 35 80 020].*WS,'ToolTipString','finish and exist');
% uicontrol(fg,'Style','Pushbutton','String','Reset...','Callback','DPARSFA_spm_image(''resetorient'')',...
...
st.mp = uicontrol(fg,'Style','edit', 'Position',[110 295 135 020].*WS,'String','','Callback','PreSurgMappProcessing_spm_image_ROI(''setposmm'');','ToolTipString','move crosshairs to mm coordinates');
st.vp = uicontrol(fg,'Style','edit', 'Position',[110 275 135 020].*WS,'String','','Callback','PreSurgMappProcessing_spm_image_ROI(''setposvx'');','ToolTipString','move crosshairs to voxel coordinates');
...
st.zoomer = uicontrol(fg,'Style','popupmenu' ,'Position',[315 75 125 20].*WS,...
    ...
    'Callback','PreSurgMappProcessing_spm_image_ROI(''zoom_in'')','ToolTipString','zoom in by different amounts');
c = 'if get(gco,''Value'')==1, spm_orthviews(''Space''), else, spm_orthviews(''Space'', 1);end;PreSurgMappProcessing_spm_image_ROI(''zoom_in'')';
...
st.win = uicontrol(fg,'Style','popupmenu','Position',[315 35 125 20].*WS,...
    'String',char('Auto Window','Manual Window'),'Callback','PreSurgMappProcessing_spm_image_ROI(''window'');','ToolTipString','range of voxel intensities displayed');
...
st.blobber = uicontrol(fg,'Style','pushbutton','Position',[450 35 125 20].*WS,...
    'String','Add Blobs','Callback','PreSurgMappProcessing_spm_image_ROI(''addblobs'');','ToolTipString','superimpose activations');
end;
return;
